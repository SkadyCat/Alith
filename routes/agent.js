...
function updateDialogueState(sessionId, fields) {
  ...
  const filePath = path.join(DIALOGUE_DIR, sessionId);
...
// ── 持久化状态文件 ────────────────────────────────────────
// 记录每个会话的启动状态、PID、端口，爱丽丝重启后仍能恢复 UI 状态
const SESSION_STATE_FILE = path.join(RUNTIME_DIR, 'agent_sessions_state.json');
...
function saveSessionState(sid, sess) {
  try {
    fs.mkdirSync(RUNTIME_DIR, { recursive: true });
    const all = loadPersistedStates();
    all[sid] = {
      isLaunched: !!sess.isLaunched,
      pid:        sess.agentProcess ? sess.agentProcess.pid : (sess._lastPid || null),
      port:       7439,
      status:     sess.agentStatus || 'idle',
      launchedAt: sess.launchedAt || null,
      updatedAt:  Date.now(),
    };
    fs.writeFileSync(SESSION_STATE_FILE, JSON.stringify(all, null, 2), 'utf-8');
  } catch (_) {}
}
...
function getSession(id) {
  ...
  if (!sessions.has(sid)) {
    ...
    // 尝试从持久化文件恢复 isLaunched 状态（不依赖 PID 存活，爱丽丝重启后子进程会被杀死）
    const persisted = loadPersistedStates();
    if (persisted[sid]) {
      const p = persisted[sid];
      // 只恢复有意义的 launched 状态：waiting/running 时保留按钮禁用状态，让用户手动停止
      if (p.isLaunched && (p.status === 'waiting' || p.status === 'running')) {
        state.isLaunched = true;
        state.launchedAt = p.launchedAt;
        state._lastPid   = p.pid || null;
        // 若之前是 running（进程被杀），统一设为 waiting（等待用户手动停止）
 ...
 */
// ── 智能编码检测：优先 UTF-8，遇到替代字符时回退 GBK ──────
// 用于处理 Windows cmd.exe (OEM/GBK) 输出混入 Copilot CLI 输出流的情况
let _iconvLite = null;
function getIconv() {
  if (_iconvLite) return _iconvLite;
  try { _iconvLite = require('iconv-lite'); } catch (_) {}
  return _iconvLite;
}
function smartDecodeBuffer(buf) {
  const utf8 = buf.toString('utf8');
  if (!utf8.includes('\uFFFD')) return utf8;  // 纯 UTF-8，无替代字符
  // 有替代字符 → 尝试 GBK 解码（中文 Windows OEM 编码）
  const iconv = getIconv();
  if (!iconv) return utf8;
  try {
    const gbk = iconv.decode(buf, 'gbk');
    // 若 GBK 解码后包含更多有效中文字符（而非替代字符），则使用 GBK 结果
    const gbkFFFD = (gbk.match(/\uFFFD/g) || []).length;
    const utf8FFFD = (utf8.match(/\uFFFD/g) || []).length;
    return gbkFFFD < utf8FFFD ? gbk : utf8;
  } catch (_) {
    return utf8;
  }
}
...
router.post('/start', async (req, res) => {
  ...
  sess.isLaunched   = true;   // 标记会话已启动，切换会话后 UI 仍可显示"停止"按钮
  sess.launchedAt   = Date.now();
  saveSessionState(sessionId, sess);  // 持久化到磁盘，重启后恢复
  updateDialogueState(sessionId, { isLaunched: true, agentPort: 7439 }); // 写入会话文件，前端切换时直接读取
  ...
  try {
    ...
    // 子进程启动后将 PID 持久化，供爱丽丝重启后检查进程存活
    if (sess.agentProcess && sess.agentProcess.pid) {
      sess._lastPid = sess.agentProcess.pid;
      saveSessionState(sessionId, sess);
    }
  } catch (err) {
  ...
  // StringDecoder 处理跨 chunk 的 UTF-8 多字节字符；GBK 乱码通过 smartDecodeBuffer 修复
  const stdoutDecoder = new StringDecoder('utf8');
  const stderrDecoder = new StringDecoder('utf8');

  sess.agentProcess.stdout.on('data', (chunk) => {
    // 始终调用 StringDecoder 维持流状态；若有替代字符则尝试 GBK 解码修正
    let text = stdoutDecoder.write(chunk);
    if (text.includes('\uFFFD')) {
      const gbkTry = smartDecodeBuffer(chunk);
      if (!gbkTry.includes('\uFFFD')) text = gbkTry;
    }
  ...
  sess.agentProcess.stderr.on('data', (chunk) => {
    let text = stderrDecoder.write(chunk);
    if (text.includes('\uFFFD')) {
      const gbkTry = smartDecodeBuffer(chunk);
      if (!gbkTry.includes('\uFFFD')) text = gbkTry;
    }
  ...
  sess.agentProcess.on('close', (code) => {
    ...
    updateDialogueState(sessionId, { isLaunched: true, agentPort: 7439 }); // 保持 isLaunched，用户需手动停止
...
router.post('/stop', (req, res) => {
  ...
  if (sess.agentStatus === 'waiting') {
    sess.pendingDone = null;
    sess.agentStatus = 'idle';
    sess.isLaunched  = false;
    saveSessionState(sessionId, sess);
    updateDialogueState(sessionId, { isLaunched: false });
...
router.post('/confirm', (req, res) => {
  ...
  sess.isLaunched  = false;   // 用户确认结束，清除已启动标记
  saveSessionState(sessionId, sess);
  updateDialogueState(sessionId, { isLaunched: false });
...
router.post('/set-status', (req, res) => {
  ...
  // 广播状态变化事件，让所有已连接的前端实时同步 UI（POLL waiting / running 等）
  const processAlive = !!(sess.agentProcess && sess.agentProcess.exitCode === null);
  broadcast(sid, 'session-status', {
    status: sess.agentStatus,
    task: sess.currentTask || null,
...
/* ─────────────────────────────────────────────────────────
   POST /agent/flush-state  —— 将当前内存中所有会话状态持久化到磁盘
   供爱丽丝重启前/定期调用，确保状态不丢失
───────────────────────────────────────────────────────── */
router.post('/flush-state', (req, res) => {
  let saved = 0;
  for (const [sid, sess] of sessions) {
    if (sess.isLaunched || sess.agentStatus === 'running' || sess.agentStatus === 'waiting') {
      if (sess.agentProcess && sess.agentProcess.pid) sess._lastPid = sess.agentProcess.pid;
      saveSessionState(sid, sess);
      saved++;
    }
  }
  res.json({ success: true, saved, message: `已持久化 ${saved} 个活跃会话状态` });
});
...
/* ─────────────────────────────────────────────────────────
   POST /agent/sync-peer  —— 从对端 Alice 服务同步上下文和进程状态
   Body: { sessionId, peerUrl, historyDoc? }
   功能：
     1. 从 peerUrl 拉取目标 sessionId 的进程状态（/agent/status）
     2. 若对端进程活跃，将本地会话 isLaunched 置为 true 并广播
     3. 若配置了 historyDoc，从对端拉取历史文档内容并覆盖保存到本地
─────────────────────────────────────────────────────────*/
router.post('/sync-peer', async (req, res) => {
  const { sessionId = 'default', peerUrl, historyDoc } = req.body || {};
  if (!peerUrl) return res.status(400).json({ success: false, error: 'peerUrl 为必填项' });
  ...
  try {
    ...
    let peerStatus = null;
    try {
      peerStatus = await fetchJson(`${base}/agent/status?sessionId=${encodeURIComponent(sessionId)}`);
    } catch (e) {
      return res.json({ success: false, error: `无法连接对端服务: ${e.message}` });
    }

    // 2. 根据对端状态更新本地 isLaunched
    const sess = getSession(sessionId);
    const peerAlive = !!(peerStatus.processAlive || peerStatus.isLaunched);
    if (peerAlive) {
      sess.isLaunched = true;
      sess.launchedAt = sess.launchedAt || Date.now();
      broadcast(sessionId, 'session-status', {
        status: peerStatus.status || 'waiting',
        task: peerStatus.task || 'POLL 等待中（对端）',
        processAlive: false,
        isLaunched: true,
        contextProgress: sess.contextProgress || 0,
      });
    ...
    if (historyDoc) {
      try {
        const histPath = historyDoc.startsWith('history/') ? historyDoc : ('history/' + historyDoc);
        const fileData = await fetchJson(`${base}/api/file?path=${encodeURIComponent(histPath)}`);
        if (fileData.success && fileData.content) {
          const relDoc = historyDoc.replace(/^history\//, '');
          const localHistPath = path.join(HISTORY_DIR, relDoc);
          fs.mkdirSync(path.dirname(localHistPath), { recursive: true });
          fs.writeFileSync(localHistPath, fileData.content, 'utf-8');
          historyFetched = true;
          historySaved = true;
        }
      } catch (_) {}
    ...
    res.json({
      ...
      historyFetched,
      historySaved,
      localIsLaunched: sess.isLaunched,
    });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
// ── POE-AI App — serve static files from application/poe-ai ──
const POE_AI_DIR = path.join(__dirname, '..', 'application', 'poe-ai');
router.use('/poe-ai', express.static(POE_AI_DIR));

// ── PyAgent 服务启动端点 ───────────────────────────────────────
// 启动独立的 PyAgent TCP 服务（pyagent_server.js on port 7441）
const PYAGENT_SERVER_SCRIPT = path.join(__dirname, '..', 'pyagent_server.js');

let _pyagentLaunching = false;

async function waitForPyAgentReady(host, port, maxWaitMs = 8000, intervalMs = 500) {
  ...
  return new Promise((resolve) => {
    const tryConnect = () => {
      ...
      sock.on('error', () => {
        sock.destroy();
        waited += intervalMs;
        if (waited >= maxWaitMs) { resolve(false); }
...
router.post('/launch-pyagent', async (req, res) => {
  const net = require('net');
  ...
  const alive = await new Promise((resolve) => {
    ...
    sock.connect(PYAGENT_PORT, PYAGENT_HOST, () => { sock.destroy(); resolve(true); });
    sock.on('error', () => { sock.destroy(); resolve(false); });
    sock.on('timeout', () => { sock.destroy(); resolve(false); });
  });

  if (alive) {
    return res.json({ success: true, launched: false, message: 'PyAgent 服务已在运行' });
  }

  if (_pyagentLaunching) {
    // 等待已有启动操作完成
    const ready = await waitForPyAgentReady(PYAGENT_HOST, PYAGENT_PORT, 6000);
    return res.json({ success: ready, launched: false, message: ready ? 'PyAgent 已就绪' : '启动超时' });
  }

  _pyagentLaunching = true;
  try {
    ...
    const ready = await waitForPyAgentReady(PYAGENT_HOST, PYAGENT_PORT);
    _pyagentLaunching = false;

    if (ready) {
      res.json({ success: true, launched: true, message: 'PyAgent 服务已成功启动' });
    } else {
      res.status(503).json({ success: false, message: 'PyAgent 启动超时' });
    }
  } catch (err) {
    _pyagentLaunching = false;
    res.status(500).json({ success: false, error: err.message });
